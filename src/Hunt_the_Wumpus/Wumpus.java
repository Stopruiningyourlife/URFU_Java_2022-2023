...
public class Wumpus extends JPanel {
    enum Hazard {
        Wumpus("Вы чувствуете ужасную вонь"),
        Bat("Вы слышите шелест перепончатых крыльев"),
        Pit("Вы ощущаете поток воздуха");
    ...
    void situation() {
        ...
        if (set.contains(Hazard.Wumpus)) {
            messages.add("Вас съел Вампус!");
            gameOver = true;

        } else if (set.contains(Hazard.Pit)) {
            messages.add("Вы упали в яму!");
            gameOver = true;

        } else if (set.contains(Hazard.Bat)) {
            messages.add("Гигантские летучие мыши уронили Вас в случайную комнату!");
    ...
    void shoot(int room) {
        if (hazards[room].contains(Hazard.Wumpus)) {
            messages.add("Победа! Вы убили Вампуса!");
            gameOver = true;

        } else {
            numArrows--;
            if (numArrows == 0) {
                messages.add("У Вас закончились стрелы.");
                gameOver = true;
                ...
                if (wumpusRoom == currRoom) {
                    messages.add("Вы разбудили Вампуса и он Вас сожрал.");
                    gameOver = true;

                } else {
                    messages.add("Вы разбудили Вампуса и он передвинулся");
    ...
    void drawStartScreen(Graphics2D g) {
        ...
        g.drawString("Охота на Вампуса", 160, 240);

        g.setFont(getFont());
        g.drawString("ЛКМ для движения, ПКМ для стрельбы", 210, 310);
        g.drawString("Учтите, что опасности могут быть в одной и той же комнате", 175, 345);
        g.drawString("Кликните чтобы начать", 310, 380);
    }
    ...
    void drawMessage(Graphics2D g) {
        if (!gameOver)
            g.drawString("Стрел  " + numArrows, 610, 30);
